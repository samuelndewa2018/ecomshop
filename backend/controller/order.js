  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      for (const [shopId, items] of shopItemsMap) {
        ...
        console.log("order", order.cart[0].images);
        ...
        const attachments = order.cart.map((item) => ({
          ...
          path: __dirname + `../../../uploads/${item.images[0]}`,
          cid: item.images[0], // Same cid value as in the html img src
        }));
        attachments.push({
          filename: "logo.png",
